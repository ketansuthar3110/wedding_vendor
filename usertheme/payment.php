...
WHERE (`tbl_booking`.`booking_id` ='{$_GET['pyd']}');";
                                    ...
                                    while ($row = mysqli_fetch_array($q)) {
                                        ...
                                        echo "</tr>";
                                        echo "<tr><td colspan=4 align=center style=background-color:skyblue;>Subtotal</td><td style=background-color:lightgreen;>{$row['booking_price']}</td></tr>";
                                    }
                                } else {
                                    ...
                                    FROM
                                        ...
                                        INNER JOIN `tbl_user` 
                                            ON (`tbl_user`.`user_id` = `tbl_booking`.`user_id`)
                                    WHERE (`tbl_vendormaster`.`vendor_id` ='{$value}');";
                                        $pq = mysqli_query($connection, $sql) or die(mysqli_error($connection));
                                        $pd = mysqli_fetch_array($pq);
                                        echo "<tr>";

                                        echo "<tr>";
                                        echo"<td>{$pd['booking_id']}</td>";
                                        echo "<td>{$pd['booking_date']}</td>";
                                        echo "<td>{$pd['user_name']}</td>";
                                        echo "<td>{$pd['vendor_name']}</td>";

                                        echo "<td>{$pd['booking_price']}</td>";

                                        echo "</tr>";
                                        echo "</tr>";
                                        $total = $pd['booking_price'] + $total;
                                    }
                                    echo "<tr><td colspan=4 align=center style=background-color:skyblue;><b>Subtotal</b></td><td style=background-color:lightgreen;><b>$total</b></td></tr>";
                                }
                                ?>
                ...
                <div class="row" style="border: 2px solid black;height: 450px;">
                    <div class="col-5 col-sm-3">
                        <div class="nav flex-column nav-tabs h-100" id="vert-tabs-tab" role="tablist"
                            ...
                            <a class="nav-link active" id="vert-tabs-profile-tab" style="color:black;" data-toggle="pill" href="#vert-tabs-profile"
                               role="tab" aria-controls="vert-tabs-profile" aria-selected="false"><b>Bank Transfer</b></a>
                            <a class="nav-link" id="vert-tabs-messages-tab" data-toggle="pill" href="#vert-tabs-messages"
                               role="tab" aria-controls="vert-tabs-messages" style="color: black;" aria-selected="false"><b>Credit / Debit</b></a>
                        </div>
                    </div>
